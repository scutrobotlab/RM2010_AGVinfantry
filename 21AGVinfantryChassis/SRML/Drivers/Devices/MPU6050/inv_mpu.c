...
#define i2c_write   IIC_Device_Write_Len
#define i2c_read    IIC_Device_Read_Len
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, ii, 1, &data))
            return -1;
        log_i("%#5x: %#5x\r\n", ii, data);
    }
    return 0;
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return i2c_read(&MPU6050_IIC_PIN,st.hw->addr, reg, 1, data);
}
...
int mpu_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    HAL_Delay(100);
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->prod_id, 1, data))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (rate == 1)
        ...
        tmp[0] = INV_LPA_640HZ;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->lp_accel_odr, 1, tmp))
        return -1;
    tmp[0] = BIT_LPA_CYCLE;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, 0x06, 6, data))
        return -1;
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s4_ctrl, 1, &div))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    if (sensors & INV_XYZ_COMPASS)
        ...
        mpu_set_bypass(0);
#else
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s1_do, 1, &data))
        return -1;
    /* Enable/disable I2C master mode. */
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
#endif
#endif
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_status, 1, data))
            return -1;
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        HAL_Delay(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, 0x0D, 3, tmp))
        return 0x07;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    HAL_Delay(200);
    data[0] = 0;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        i2c_write(&MPU6050_IIC_PIN,st.hw->addr, 0x23, 1, &tmp);
...
int setup_compass(void)
{
    ...
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s0_addr, 1, data))
        return -1;

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s0_reg, 1, data))
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s1_addr, 1, data))
        return -1;

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s1_reg, 1, data))
        return -1;

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->s1_ctrl, 1, data))
        return -1;
    ...
    data[0] = 0x03;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))
        return -1;

#ifdef MPU9150
    /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
    data[0] = BIT_I2C_MST_VDDIO;
    if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->yg_offs_tc, 1, data))
        return -1;
#endif
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    if (i2c_read(&MPU6050_IIC_PIN,st.hw->addr, st.reg->raw_compass, 8, tmp))
        return -1;
#endif
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->motion_thr, 2, data))
            goto lp_int_restore;
        ...
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->lp_accel_odr, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (MPU6500 version). */
        data[0] = BITS_WOM_EN;
        if (i2c_write(&MPU6050_IIC_PIN,st.hw->addr, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
...
unsigned char mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	if(sensors&INV_WXYZ_QUAT)
	{
		...
		q3 = quat[3] / q30;
		//计算得到俯仰角/横滚角/航向角
		*pitch = (float)asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3f;	// pitch
		*roll  = (float)atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3f;	// roll
		*yaw   = (float)atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3f;	//yaw
	}else return 2;
	return 0;
}
